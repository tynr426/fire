...
import fire.web.utils.VerifyCodeUtils;
@Controller
public class VerifyCodeController{
	    ...
	    public String getSysManageLoginCode(HttpServletResponse response,
	        ...
	        response.setContentType("image/jpeg");// 设置相应类型,告诉浏览器输出的内容为图�?
	        response.setHeader("Pragma", "No-cache");// 设置响应头信息，告诉浏览器不要缓存此内容
	        response.setHeader("Cache-Control", "no-cache");
	        response.setHeader("Set-Cookie", "name=value; HttpOnly");//设置HttpOnly属�??,防止Xss攻击
	    ...
	    public String checkTcode(HttpServletRequest request,HttpServletResponse response) {
	        ...
	        return "error";
	        //这里我没有进行字母大小模糊的验证处理，感兴趣的你可以去试�?下！
	    }
}
