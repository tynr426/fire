...
var wechat = {
		...
		set: {
			load:function(){
				$.ajax({
					url:path+"/admin/social/getWeChatAccount.do",
					type:"post",
					dataType:"json",
					success:function(result){
						...
						if(result.state==0){
							...
							else
							{
								...
								$("#div_token").html(result.data.token);
								$("#div_key").html(result.data.encodingAESKey);
						...
						if(result.data.length==0){
							result.data.push({name:"",catering:0});
						}
			...
			save: function () {
				...
				var OriginalId = $("#OriginalId").val().trim();
				var Account = $("#Account").val().trim();
				var Catering = $("#Catering").val();
				var AppId = $("#AppId").val().trim();
				var Secret = $("#Secret").val().trim();
				$.ajax({
					url:path+"/admin/social/save.do",
					type:"post",
					data:{Id:Id,Name:Name,OriginalId:OriginalId,
						Account:Account,Catering:Catering,AppId:AppId,
						Secret:Secret},
					dataType:"json",
					success:function(result){
						if(result.state==0){
