...
uploadHandler.fn = uploadHandler.prototype = {
    ...
    defaults: (function () {
        return {
            ...
            default_preview: path+"/Static/Images/error.png", //默认的预览图片
            max_picture: "", 	//给定初始的图片
...
uploads.fn = uploads.prototype = {
    ...
    init: function (container, config) {
		...
		.replace(/\{\$src\$\}/g, config.default_preview == undefined ? path+"/Static/Images/error.png" : config.default_preview)//替换默认预览图片
    ...
    _init: (function () {
        uploads["innerUploads"] = '<script language="javascript">\
			...
			<!--大图预览-->\
			<div class="max_pic_box" style="display:none;" >{$isDelete$}<img id="{$id$}_max_pic_img" src="{$src$}" alt="暂无预览" onload="javascript:$(this).scaleZoom({$pwidth$},{$pheight$});" nolazy="0" /></div>\
			<!--缩略图-->\
            <div class="clear"></div>\
